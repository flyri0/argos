...
- Every syncable row carries a **Hybrid Logical Clock (HLC)** timestamp instead of a plain wall-clock time, and `server_version` (assigned by the server on write, monotonically increasing). See §5 for the exact columns.
...
- Each device stores a copy of the server's `sync_id` (see §5, `server_meta`). If the server's `sync_id` ever changes — a restore from backup, a manual reset — the client detects the mismatch and re-downloads the full dataset instead of attempting to merge against a server history it no longer shares.
- **Partial failure within a push**: a single `/sync` push can carry mutations accumulated over days offline, touching many unrelated rows. Failing the entire push because one mutation is invalid (e.g. it references a category deleted by another device in the meantime) would block every other valid change in that batch from reaching the server — bad odds the longer a device has been offline. So the server applies mutations **independently by default** and reports success/failure per mutation, not per batch. The exception is mutations that are only meaningful together — the two sides of a transfer (linked by `transfer_id`), or a category/payee delete alongside its `reassign_to` move (§5.4) — the client submits these as a **group**, and the server commits or rejects each group atomically, while still treating different groups in the same push independently of each other.
...
- Both modes read the same config file, so switching a machine from desktop use to headless service use doesn't require reconfiguration.
...
- **Server**: English-only. The API returns stable, machine-readable error codes (e.g. `INSUFFICIENT_FUNDS`, `INVALID_DATE`) alongside a plain English message for logs/debugging — it never returns pre-translated text. Localization of anything user-facing is entirely the frontend's responsibility: the client maps error codes to a friendly, translated message using its own locale files. This keeps the server simple and avoids duplicating translation logic on both sides.
...
devices                          -- see §6, Authentication & device pairing. Not synced to clients.
  id             uuid primary key
  name           text           -- user-editable label, e.g. "Ana's phone"
...
- The first device that presents the code is auto-approved and becomes the first trusted device — no prior approval needed, since none can exist yet. That single use is what closes the window, not a clock: the code is consumed on first use and cannot pair a second device. Every device after that first one must go through normal approval (§6.2) from an already-trusted device.
...
- **First contact**: a device hitting the API without a valid token gets a `403 PAIRING_REQUIRED`. The frontend shows a "waiting for approval" screen with a short pairing code.
- **Approval**: the *already-trusted* device (or the desktop tray app) shows a prompt — "New device requesting access: code `4821`, approve?" — and the user confirms there. Approval mints a long-lived token for the new device and records it in `devices` (§5.2).
...
- **Managing devices**: the tray app (desktop mode) and a settings page (PWA) both list paired devices with last-seen time, and allow revoking any of them, which immediately invalidates that device's token.
...
Both the bootstrap setup code (§6.1) and the per-request pairing code (§6.2) are secrets guessed over the network, so both endpoints (`/api/pairing/request` code submission and `/api/pairing/approve`) enforce a per-source lockout: after a small number of failed attempts (e.g. 5) from the same IP, further attempts are delayed with exponential backoff, then temporarily blocked outright. This is what actually makes the setup code's lack of time-based expiry safe — entropy plus a slow guessing rate together make brute-forcing impractical, where either alone would not be enough.
...
| `PUT` | `/api/budget/:month/:category_id` | Set budgeted amount |
...
| `POST` | `/sync` | Push local outbox mutations, pull remote changes since cursor |
| `GET` | `/health` | Health check |

## 8. MVP feature checklist

- [ ] Accounts: create, edit, close (on-budget / off-budget); balance always computed from transactions, never stored
- [ ] Transactions: create, edit, delete, mark cleared, transfers between accounts
...
- [ ] Budgeting: assign amounts per category/month, correct rollover, correct overspending rule, "Available to Budget" calculation
...
- [ ] Sync push: per-mutation success/failure reporting, with explicit atomic groups for transfers and category/payee reassignment
...
- [ ] Device pairing: first-device bootstrap via high-entropy setup code (no expiry, rate-limited), approval flow for subsequent devices, token issuance, device list + revocation, localhost bypass
