...
- The client's outbox is drained to the server's `/sync` endpoint whenever connectivity is detected — concretely, on three triggers: the browser's `online` event (this device's own connectivity being restored), the tab regaining visibility (switching back to Argos shouldn't require waiting out a poll interval to see what changed elsewhere), and a periodic poll every 10 seconds while online. The periodic poll is what actually keeps multiple devices converged without a manual page reload: a reconnect/visibility event alone only catches this device's own state changes, not another device's write landing on the server in the meantime.
...
- **Persisted client clock**: each device persists its latest HLC (physical, counter) locally, shared by all tabs of that device, so monotonicity and everything the device has observed survive reloads — otherwise a device whose clock is behind would, after a reload, stamp edits lower than rows it already pulled and have them rejected as stale, and two tabs sharing one node id could emit identical triples. On first start without a persisted value, the device seeds its clock from the greatest HLC present in its local replica before syncing or accepting writes.
...
- **Referential rules on /sync**: the server enforces §5.4 on `/sync`, not only on REST, since another device may have used a category, payee, or account offline before a delete reaches it. A delete of a category with non-deleted transactions or `budget_entries` referencing it is `rejected_invalid` with `CATEGORY_IN_USE_NEEDS_REASSIGN`; a delete of a payee with non-deleted referencing transactions, with `PAYEE_IN_USE_NEEDS_REASSIGN`; a delete of an account with non-deleted transactions, with `ACCOUNT_IN_USE`. These checks run after the earlier mutations of the same unit, so a reassignment group that moves the references first passes. An upsert that points `transactions.account_id`/`category_id`/`payee_id`, `budget_entries.category_id`, or `categories.group_id` at a soft-deleted row is `rejected_invalid` with `REFERENCE_DELETED` — unless the stored row already has exactly that reference, so edits to rows that already point at a tombstone don't get stuck.
- **One set of rules for both write paths**: the REST endpoints (§7.3) and `/sync` enforce the same rules — HLC last-write-wins, the transfer pair invariant (§5.2), budget entry identity (§5.2), and §5.4 — so a second write path (or any future server-side writer, e.g. CSV import) can't overwrite newer data or break an invariant `/sync` protects. The one intentional difference: REST `POST /api/transactions` rejects closed accounts (§7.1) while `/sync` accepts writes to them, because an offline device cannot know an account was closed.
...
- **Conflict rejections**: some `rejected_invalid` results are concurrent conflicts rather than malformed data. The conflict codes are `REFERENCE_DELETED`, `CATEGORY_IN_USE_NEEDS_REASSIGN`, `PAYEE_IN_USE_NEEDS_REASSIGN`, `ACCOUNT_IN_USE`, and `TRANSFER_PAIR_INVALID`. For a unit rejected with a conflict code, the server includes the current row for every row the unit referenced in `changes`, exactly as for `rejected_stale`. The client treats it as terminal: it applies those rows, purges local-only rows by the same rule as `rejected_stale`, marks the entries synced, and shows a conflict notice. Every other `rejected_invalid` code (`SYNC_MUTATION_INVALID`, `CLOCK_SKEW_TOO_LARGE`, `INTERNAL_ERROR`) keeps being retried, since it points at a client bug or a bad clock rather than a conflict another device can resolve.
...
- `changes`: every row, across all syncable tables, with `server_version` greater than the request's `since` — this is the pull half of the same round-trip. In addition, regardless of `since`, it includes the current server row for every row referenced by a unit reported `rejected_stale` (rows that don't exist on the server are omitted). Without this, a losing device whose cursor is already past the winning row — or whose group was rolled back because one member was stale — would never receive the state it lost to. Each row appears at most once. Because every existing row a `rejected_stale` unit referenced is returned, a referenced row absent from `changes` does not exist on the server: the client removes it from its local replica if it was never acknowledged (no `server_version`) and no other unsynced outbox entry references it. This is a local purge of a row the server never had — like the tombstone purge in §2.3 — not a hard delete of synced data.
...
**Starting balance on account creation**: since `accounts` deliberately has no `balance` column (above), a new account with pre-existing funds has nothing to hold that value. Mirroring YNAB and Actual Budget, the account creation form offers an optional **starting balance** field as pure client-side convenience: if a non-zero amount is entered, the client creates the account as normal and then immediately creates one ordinary transaction on it (`category_id` and `payee_id` both null, `date` the day of creation, `notes` "Starting Balance", `cleared` true) for that amount — the same outcome a user could reach by hand via a regular transaction. A zero or blank starting balance creates no transaction at all. This introduces no new column, no new endpoint, and no new wire-format shape; the two writes are ordinary, independent mutations (not a §2.4 atomic group) since an account existing without its opening transaction having synced yet is not a broken invariant the way an unpaired transfer leg would be.
...
These rules apply to `/sync` as well as REST (§2.3): a standalone category or payee delete mutation is rejected if the row is still in use when it reaches the server — `CATEGORY_IN_USE_NEEDS_REASSIGN` or `PAYEE_IN_USE_NEEDS_REASSIGN`, nested in the `/sync` result — while a delete grouped with the mutations that move its references (§2.4) is checked after those moves and passes. Account deletes are held to the same rule (`ACCOUNT_IN_USE`), and no `/sync` upsert may point a reference at a soft-deleted row (`REFERENCE_DELETED`, §2.3).
...
- **Stale REST writes**: a REST update or delete whose HLC is not strictly greater than the stored row's HLC returns `409 STALE_WRITE` and leaves the row unchanged (§2.3). An operation touching several rows — a delete with `reassign_to`, which moves transactions and `budget_entries`, or a transfer edit/delete, which writes both legs — is rejected entirely if any row it would modify has an equal or newer HLC.
- `DELETE /api/transactions/:id` on a transfer leg soft-deletes both legs (§5.2's pair invariant).
- `PATCH /api/transactions/:id` on a transfer leg mirrors `date`, `payee_id`, `notes`, and `amount` (negated) to the sibling leg; `cleared` applies to that leg only. Changing `account_id` or `category_id` on a transfer leg returns `400 VALIDATION_ERROR`.
...
| `CATEGORY_IN_USE_NEEDS_REASSIGN` | 409 (REST); also nested in a `/sync` result for a delete mutation (§2.4) | category delete requested without `reassign_to` (REST), or without first moving its references in the same unit (`/sync`), but it's in use (§5.4) |
| `PAYEE_IN_USE_NEEDS_REASSIGN` | 409 (REST); also nested in a `/sync` result for a delete mutation (§2.4) | payee delete requested without `reassign_to` (REST), or without first moving its references in the same unit (`/sync`), but it's in use (§5.4) |
...
| `STALE_WRITE` | 409 | a REST update or delete carried an HLC not strictly greater than a row it would modify (§7.1) |
| `ACCOUNT_IN_USE` | n/a — nested in a `/sync` result, not a top-level status (§2.4) | a `/sync` delete of an account that still has non-deleted transactions (§2.3) |
| `REFERENCE_DELETED` | n/a — nested in a `/sync` result, not a top-level status (§2.4) | a `/sync` upsert points a reference (`account_id`, `category_id`, `payee_id`, `group_id`) at a soft-deleted row the stored row didn't already reference (§2.3) |
